...
def hello():
    time.sleep(1)
    print("Welcome to day 15 of 🐍 challenge! lets keep rocking ")

hello()

"""**🐍 Day 16 of Python Challenge**"""
...
with SafeFileHandler("example.txt", "w") as f:
    f.write("Hello, Welcome to 30Days Of 🐍 Challenge.")

"""**🐍 Day 18 of Python Challenge**"""
...
class MethodNameChecker(type):
    def __new__(cls, name, bases, attrs):
        ...
        if bad:
            raise TypeError(f"🚫 Invalid method names in '{name}': {bad}. "
                            "Method names (excluding dunder methods) must be lowercase.")
        return super().__new__(cls, name, bases, attrs)

# --- Demonstrations ---

# ✅ Valid Class: Methods are all lowercase
class Good(metaclass=MethodNameChecker):
    def hello(self): pass
    def show_info(self): pass

print("✅ 'Good' class defined successfully.")

# ❌ Invalid Class: 'Hello' method is not lowercase, will raise an error
try:
...
"""**🐍 Day 23 of Python Challenge**"""

import ipywidgets as widgets
from IPython.display import display
temp = widgets.FloatText(description='Temp:')
convert_type = widgets.Dropdown(options=["C→F", "F→C"], description='Convert:')
output = widgets.Label()
def convert_temp(change):
    t = temp.value
    if convert_type.value == "C→F":
        result = (t * 9/5) + 32
    else:
        result = (t - 32) * 5/9
...
with gr.Blocks() as demo:
    gr.Markdown("📚 **Simple Book Library**")
    with gr.Row():
    ...
    show_btn = gr.Button("📚 Show All Books")
    output_all = gr.JSON()
    show_btn.click(fn=get_books, outputs=output_all)
demo.launch()

"""**🐍 Day 27 of Python Challenge**"""
...
def delete_book(book_id):
    ...
    if book:
        db.delete(book)
        db.commit()
        msg = f"🗑️ Deleted Book ID {book_id}"
    else:
        msg = f"❌ Book ID {book_id} not found"
    db.close()
    return msg
# 🎨 Gradio Interface
with gr.Blocks() as demo:
    gr.Markdown("# 📚 Book Manager")
    with gr.Row():
    ...
    with gr.Row():
        get_btn = gr.Button("📖 Show All Books")
        delete_id = gr.Number(label="Book ID to Delete", precision=0)
        delete_btn = gr.Button("🗑️ Delete Book")
